...
class Hamburger {
  ...
  removeTopping(topping) {
  	this._toppings.includes(topping) ? this._toppings.filter(item => item !== topping) : null;
  }
   ...
   *
   * Попробуйте сделать это геттером чтобы можно было обращаться как obj.price 
   и нам вернет сумму.
   */
  get price() {
  	let sizePrice = Hamburger.SIZES[this._size].price;
  	let stuffingPrice = Hamburger.STUFFINGS[this._stuffing].price;
  	let toppingsPriceArray = this.toppings.map(topping => Hamburger.TOPPINGS[topping].price)
  	let toppingsPriceTotal = toppingsPriceArray.reduce((acc, num)=> acc += num,0)
  	const result = sizePrice + stuffingPrice + toppingsPriceTotal;
  	return result;
  }
  ...
  get calories() {
  	...
  	let toppingsCaloriesArray = this.toppings.map(topping => Hamburger.TOPPINGS[topping].calories)
  	let toppingsCaloriesTotal = toppingsCaloriesArray.reduce((acc, num)=> acc += num,0)
  	const result = sizeCalories + stuffingCalories + toppingsCaloriesTotal;
...
*/
Hamburger.SIZE_SMALL = 'S';
Hamburger.SIZE_LARGE = 'L';
...
/*
  🔔 Обратите внимание на такие моменты:
    	...
    	✔️ типы начинок обозначены "константами" с понятными именами (на самом деле просто
	    	свойствами, написанным заглавными буквами, которые мы договорились считать 
	    	"константами")
	   ✔️ объект создается через конструктор - функцию, которая задает начальные 
	   значения полей.
    	✔️ в свойствах объекта гамбургера логично хранить исходные данные 
    	(размер, тип начинки),а не вычисленные из них (цена, число калорий и т.д.).
    	Рассчитывать цену и калории	логично в тот момент, когда это потребуется, а не заранее.
*/