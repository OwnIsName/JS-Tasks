...
const getUserByEmail = (arr, email) => {
	return arr.find(user => user.email.includes(email))
};

console.log(getUserByEmail(users, 'shereeantho')); // {объект пользователя Sheree Anthony}
...
const getTotalBalance = arr => {
	const balances = arr.map(user => user.balance)
	console.log(balances)
	return balances.reduce((accum, item) => accum + item, 0)
};
...
const getUsersByFriend = (arr, name) => {
	return arr.filter(user => user.friends.includes(name))
};
...
const getAllSkills = arr => {
	const uniqueSkills = [];
	const getUniqueSkills = arr.map(user => user.skills.map(x => {
		if(uniqueSkills.includes(x)) return
		uniqueSkills.push(x)
	}));
	const sortUniqueSkills = uniqueSkills.sort()
...
function sortedArr(arr){
	let currentFriendsCount = arr
	for(let i = 0; i < arr.length; i++){
		arr[i+1].friends.length - arr[i].friends.length 
	}
}
...
const getUserNamesSortedByFriendsCount = arr => {
	const result = arr.sort(sort1).map(x => x.friends)
